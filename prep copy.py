...
class RollingScaler:
    ...
    def transform(self, X):
        ...
        else: raise ValueError("Unsupported shape for transform")
...
def prep_data(feat, ret, config, anchor_date=None):
    ...
    train_sequences, train_targets, train_dates, test_sequences, test_targets, test_dates = [], [], [], [], [], []
    ...
    alpha = config.get("Z_GLOB") ; beta =  config.get("Z_ANCH")
    ...
    if train_dates_adj: print(f"[Prep] Adj.training dates: {train_dates_adj[0].date()} - {train_dates_adj[-1].date()}")
