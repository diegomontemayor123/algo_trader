import torch, multiprocessing, copy, random
import pandas as pd
import numpy as np
...
def run_training_chunks(chunks, lback, norm_feat, TICK, feat, comp_feat, macro_keys, config, start, device, initial_capital):
    ...
    for idx, (chunk_start, chunk_end) in enumerate(chunks):
        ...
        feat_train, ret_train = comp_feat(TICK, feat_list, cached_chunk_data, macro_keys, split_date=chunk_config["SPLIT"], method=["rf"], train_end_date=train_end)
        ...
        for i in range(start_idx - lback, end_idx - lback + 1):
            ...
            train_feat = feat_df.loc[:train_end] 
            normalized_feat = (feat_win - train_feat.mean().values.astype(np.float32)) / (train_feat.std().values + 1e-10).astype(np.float32)
