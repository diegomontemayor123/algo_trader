...
def run_experiment(trial):
    ...
    config.update({
        "TICK": ",".join(select_TICK),
        "MACRO": ",".join(select_macros),
        "FEAT": ",".join(select_feat),
    })
    ...
    try:
        ...
        with open("img/tune_log.log", "a") as f:
            f.write("\n\n=== Trial output start ===\n")
            f.write(output)
            f.write("\n=== Trial output end ===\n")
        ...
        def extract_avgbenchperf(out):
            matches = re.findall(r"Average Bench OutPerf(?: Across Chunks)?:\s*([-+]?\d+(?:\.\d+)?)%", out)
