...
class DataLoader:
    ...
    def load_data(self, start=START, end=END):
        ...
        start_dt = pd.to_datetime(start)  # Convert start to datetime too
        ...
        if os.path.exists(self.cache_path):
            ...
            # Ensure index is datetime and handle comparison properly
            if hasattr(data.index, 'min') and hasattr(data.index, 'max'):
                try:
                    ...
                    # If there's any issue with date comparison, just reload
                    pass
        ...
        # Ensure the index is a proper DatetimeIndex before saving
        if not isinstance(data.index, pd.DatetimeIndex):
            data.index = pd.to_datetime(data.index)
...
class FeatureEngine:
    ...
    def create_features(self, data):
        ...
        for ticker in TICKERS:
            ...
            r = p.pct_change()  # This is a Series, not DataFrame
            ...
            if sector_etf and sector_etf in data.columns:
                try:
                    sector_ret = data[sector_etf].pct_change()  # This is also a Series
                    # Ensure both are Series and have the same index
                    if isinstance(r, pd.Series) and isinstance(sector_ret, pd.Series):
                        ...
                        print(f"Warning: Unexpected data types for {ticker} sector calculations")
                        f['sector_beta'] = 0.0
                        f['sector_alpha'] = 0.0
                except Exception as e:
        ...
        for macro in MACRO_TICKERS:
            if macro in data.columns:
                ...
                macro_feat = pd.DataFrame({'ret': m_ret, 'vol': m_vol, 'zscore': zscore(m_ret.rolling(60).mean())}, index=data.index)

                features.append(macro_feat.add_suffix(f'_{macro}'))
        X = pd.concat(features, axis=1)
        X['vix_term'] = (data.get('^TNX', pd.Series(0.03, index=data.index)) - data.get('^VIX', pd.Series(20, index=data.index)) / 100)
...
class RegimeDetector:
    ...
    def detect(self, returns):
        ...
        corr = returns.rolling(self.lookback).apply(lambda x: x.corr().values[np.triu_indices_from(x.corr().values, k=1)].mean(), raw=False)
        breadth = (returns > 0).sum(axis=1) / returns.shape[1]
        
        regime = pd.Series(0, index=returns.index)
        regime[vol > vol.quantile(0.8)] = 1
        regime[(vol > vol.quantile(0.95)) | (corr > corr.quantile(0.9)) | (breadth < 0.2)] = 2
...
class EnhancedMLStrategy:
    ...
    def fit_regime_models(self, X, y, regimes):
        for regime in [0, 1, 2]:
            ...
            scaler = RobustScaler()
            X_scaled = scaler.fit_transform(X_clean[self.selected_features].fillna(0))
    ...
    def predict(self, X, current_regime=0):
        if not self.selected_features: return pd.Series(0.0, index=X.index)
        ...
        for model in models_to_use.values():
            try:
                if hasattr(model, 'predict'): predictions.append(model.predict(X_scaled))
            except: continue
...
class HybridAlphaTrader:
    ...
    def backtest(self, start=START, end=END, split=SPLIT, retrain_freq=20):
        ...
        for i, date in enumerate(test_dates):
            ...
            if predictions.empty or predictions.iloc[0] == 0: continue
