...
def run_retraining_chunks(chunks, feat_df, ret_df, lback, norm_feat, TICK, feat, macro_keys, config, start, device, initial_capital, model0=None):
    ...
    for idx, (chunk_start, chunk_end) in enumerate(chunks):
        ...
        if idx == 0:
            ...
            original_train_length = (chunks[0][0] - pd.to_datetime(start))
            ...
            if current_model is None:
                print(f"Skipping chunk {idx+1} due to failure.")
                continue
        ...
        for i in range(start_idx - lback, end_idx - lback + 1):
            ...
            with torch.no_grad():
                weight = current_model(input_tensor).cpu().numpy().flatten()
