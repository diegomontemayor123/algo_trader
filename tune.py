...
def run_experiment(trial):
    config = {
        ...
        "TICKERS": trial.suggest_categorical("TICKERS", ["AAPL,MSFT,GOOGL,NVDA,JPM,WMT,CVX,MCD,T,NKE"]),
        "MACRO": trial.suggest_categorical("MACRO", ["FEDFUNDS,^GSPC,^DJI,^IXIC,^RUT,^FTSE,^N225,CL=F,GC=F,SI=F,NG=F,ZW=F,EURUSD=X,GBPUSD=X,USDJPY=X,^TNX,^FVX,^IRX,^TYX,TLT,IEF,GLD,USO,UUP"]),
        ...
        "L1_PENALTY": trial.suggest_float("L1_PENALTY", 1e-5, 1e-3),
    ...
    try:
        ...
        score = (1 * sharpe) + (1 * avg_benchmark_outperformance) - (0.1 * abs(drawdown))
