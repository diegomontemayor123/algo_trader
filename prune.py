...
def hash_array_safe(arr):
    """Hash a numpy array or pandas Series safely, replacing NaN/Inf for hashing."""
    arr_copy = np.nan_to_num(arr.values if isinstance(arr, pd.Series) else arr,
                             nan=0.0, posinf=1e10, neginf=-1e10)
    return hashlib.md5(np.ascontiguousarray(arr_copy, dtype=np.float64)).hexdigest()

def log_array(name, arr):
    """Print safe summary of a pandas Series or DataFrame for debugging."""
    if isinstance(arr, pd.Series):
        print(f"[Debug] {name} head:\n{arr.head().fillna(0)}")
        print(f"[Debug] {name} stats: min {arr.min()}, max {arr.max()}, mean {arr.mean()}, count {arr.count()}")
    elif isinstance(arr, pd.DataFrame):
        print(f"[Debug] {name} shape: {arr.shape}")
        print(f"[Debug] {name} head:\n{arr.head().fillna(0)}")
        print(f"[Debug] {name} hash: {hash_array_safe(arr)}")
...
def select_features(feat, ret, split_date, thresh=config["THRESH"], method=["rf"]):
    ...
    log_array("Step 1: masked_ret", masked_ret)
    
    portfolio_ret = masked_ret.mean(axis=1, skipna=True)
    log_array("Step 2: portfolio_ret", portfolio_ret)
