...
class PriceLoader:
    ...
    def load(self, start=START, end=END) -> pd.DataFrame:
        ...
        if os.path.exists(self.cache_path):
            ...
            if df.index.min() <= pd.to_datetime(start) and df.index.max() >= end_dt:
        ...
        for t in all_tickers:
            try:
                ...
                cols[(t,'vol')]   = d.get('Volume', pd.Series(1e6, index=d.index))
                cols[(t,'high')]  = d.get('High', d['Close'])
                cols[(t,'low')]   = d.get('Low', d['Close'])
            except Exception:
                continue
        if not cols:
    ...
    def build(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for t in TICKERS:
            ...
            if sec and (sec,'close') in data:
                ...
                f['sector_beta'] = p.pct_change().rolling(60).corr(sec_ret)
                f['sector_alpha'] = p.pct_change().rolling(20).mean() - sec_ret.rolling(20).mean()
        ...
        for m in MACRO_TICKERS:
            if (m,'close') in data:
                ...
                f[f'{m}_z60'] = zscore(mret.rolling(60))
                feats.append(f)
...
class RegimeDetector:
    ...
    def detect(self, returns: pd.DataFrame) -> pd.Series:
        ...
        corr = returns.rolling(self.lookback).apply(lambda _: avg_corr(_), raw=False)
...
class PortfolioConstructor:
    ...
    def optimize(self, alpha: Dict[str, float], returns: pd.DataFrame, current: Optional[Dict[str, float]] = None, regime: int = 0) -> Dict[str,float]:
        ...
        port_vol = math.sqrt(np.dot(np.array([wdict[s] for s in symbols]), cov @ np.array([wdict[s] for s in symbols])))
...
class Backtester:
    ...
    def run(self,
        ...
        for i, d in enumerate(dates):
            if d not in features.index or d not in prices.index:
                continue
            ...
            if last_train is None or (d - last_train).days >= retrain_freq_days:
                ...
                Rtr = returns.loc[train_start:train_end]
            ...
            if not sig:
                ...
                continue
            # optimize
            ret_hist = prices[[ (t,'close') for t in TICKERS if (t,'close') in prices]].droplevel(1, axis=1).pct_change()
            target = builder.optimize(sig, ret_hist.loc[:d], current=weights, regime=int(reg))
            if not target:
                equity.append((d, eq))
                continue
            # apply costs
            ...
            slip_cost = (self.slip/1e4) * sum(abs(target.get(t,0.0) - weights.get(t,0.0)) for t in target)

            # hold until next rebalance
            next_d = dates[i+1] if i+1 < len(dates) else d + BDay(TARGET_HORIZON_DAYS)
            rets = returns.loc[d:next_d, list(target.keys())]
            period_ret = float((rets * pd.Series(target)).sum(axis=1).sum())
            eq *= (1.0 + period_ret - tc_cost - slip_cost)
            equity.append((d, eq))
            weights = target
...
class HybridInstitutionalTrader:
    ...
    def run(self, start=START, end=END, split=SPLIT) -> Dict[str,object]:
        ...
        close_cols = [(t,'close') for t in TICKERS if (t,'close') in data]
