...
class DataLoader:
    ...
    def load_data(self, start=START, end=END):
        ...
        if os.path.exists(self.cache_path):
            ...
            if (data.index.min() <= pd.to_datetime(start)) and (data.index.max() >= end_dt):
                return data.loc[start:end]
        
        raw = {}
        for ticker in all_tickers:
            try:
                df = yf.download(ticker, start=pd.to_datetime(start) - pd.Timedelta(days=400), end=end_dt, auto_adjust=True)
                if not df.empty:
                    raw[ticker] = df['Close']
    ...
    def create_features(self, data):
        ...
        for macro in MACRO_TICKERS:
            if macro in data.columns:
                ...
                macro_feat = pd.DataFrame({'ret': m_ret, 'vol': m_vol, 'zscore': zscore(m_ret.rolling(60))}, index=data.index)
