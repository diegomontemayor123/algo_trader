...
def run_experiment(trial,study=None):
    config = {"START": trial.suggest_categorical("START", ["2012-07-01","2012-04-01","2012-01-01","2011-10-01"]),#2019 Jan
        ...
        "YWIN": trial.suggest_int("YWIN",28,35),#29,28
        "PRUNEWIN": trial.suggest_int("PRUNEWIN",30,40),#31,28
        "PRUNEDOWN": trial.suggest_float("PRUNEDOWN",1.3,1.6),
        "THRESH": trial.suggest_int("THRESH",230,300),#170
        "NESTIM": trial.suggest_int("NESTIM",200,200),#200
        "BATCH": trial.suggest_int("BATCH",51,60),#53
        "LBACK": trial.suggest_int("LBACK",75,85),#84
        "PRED_DAYS": trial.suggest_int("PRED_DAYS",5,5),#6
        "DROPOUT": trial.suggest_float("DROPOUT",.035,.04),#.035
        "DECAY": trial.suggest_float("DECAY",.0033,.0039,log=True),#.003
        "SHORT_PER": trial.suggest_int("SHORT_PER",7,14),#12
        "MED_PER": trial.suggest_int("MED_PER",14,25),#17
        "LONG_PER": trial.suggest_int("LONG_PER",40,60),#58
        "INIT_LR": trial.suggest_float("INIT_LR",.004,.006,log=True),#.001
        "EXP_PEN": trial.suggest_float("EXP_PEN",.24,.25),#.24
        ...
        "MAX_HEADS": trial.suggest_categorical("MAX_HEADS",[1,2,3]),#1
        "LAYERS": trial.suggest_categorical("LAYERS",[5,6,7,8,9]),#2
        "EARLY_FAIL": trial.suggest_categorical("EARLY_FAIL",[4,5,6,7,8]),#2
        "VAL_SPLIT": trial.suggest_categorical("VAL_SPLIT",[.15]),#.15
        "TEST_CHUNK": trial.suggest_categorical("TEST_CHUNK",[6,10,12,14]),
